...
const SingleLightGroup = (props) => {
    ...
    const [isPerformingLightAction, setIsPerformingLightAction] = useState(
        false
    );
    ...
    const toggleGroup = () => {
        ...
        backend.get(apiPath).then(async () => {
            setShouldFetchGroup(!shouldFetchGroup);
        });
    };

    const toggleLight = (lightId) => {
        backend.get(`/hue/lights/${lightId}/toggle`).then(async () => {
            setShouldFetchGroup(!shouldFetchGroup);
        });
    };

    const activateScene = (sceneId) => {
        backend.get(`/hue/scenes/${sceneId}/activate`).then(async () => {
            setShouldFetchGroup(!shouldFetchGroup);
        });
    };

    useEffect(() => {
        backend.get(`/hue/groups/${props.id}/scenes`).then((response) => {
    ...
    useEffect(() => {
        backend
            .get(`/hue/groups/${props.id}`)
            .then((response) => {
                setCurrentLightGroup(response.data._data);
                return backend.get(
                    `/hue/lights/list?lightIds=${response.data._data.lights.join()}`
                );
            })
            .then((response) => {
