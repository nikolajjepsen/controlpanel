...
const Light = ({ id, name, reachable, currentLightState, bri, onToggleLight }) => {
    return (
        <div
            id={`light_${id}`}
            className={`light ${reachable ? '' : 'unreachable'} ${
                currentLightState === 'on' ? 'active' : ''
            }`}
            onClick={(e) => (reachable ? onToggleLight(e, id) : false)}
        >
            ...
            <div className={`light-details`}>
                ...
                {currentLightState === 'on' && (
                    <span className="state">On at {Math.ceil((Number(bri) / 254) * 100)}%</span>
                )}
                {currentLightState === 'off' && reachable && <span className="state">Off</span>}
                {currentLightState === 'off' && !reachable && (
                    <span className="state">Unreachable</span>
                )}
            </div>
            {currentLightState === 'on' && <div className="brightness-slider"></div>}
        </div>
    );
};
