...
import { faBed, faCouch, faDoorOpen, faMoon, faLayerGroup } from "@fortawesome/free-solid-svg-icons";
...
const LightGroup = (props) => {
    ...
    useEffect(() => {
        ...
        setIsLoading(true);
		backend.get(
            `/hue/lights/list?lightIds=${props.lights.join()}`
        )
        .then((response) => {
    ...
    return (
		<Col
			xs={{span: props.expanded ? 12 : 6, order: props.expanded ? "first" : "",}}
			lg={{span: props.expanded ? 12 : 4, order: props.expanded ? "first" : "",}}
			className="mb-4"
			key={props.groupId}
		>
			<div className="light-group">
				...
				>
					...
					<p className="active-devices">
						{props.deviceCount} devices reachable.
					</p>
				</div>
				<Row className="mx-3">
					<Col xs={12}>
