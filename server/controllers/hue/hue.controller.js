...
const listLights = async (req, res, next) => {
    ...
    hue.api.createLocal(ip).connect(config.username)
        .then((api) => {
        ...
        .then((lights) => {
            ...
            if (lightsArray.length > 0) {
                ...
                filteredLights = lights.filter(
                    (light) => lightsArray.includes(light._data.id.toString())
                );
            } else {
...
const listGroups = async (req, res, next) => {
    ...
    hue.api.createLocal(ip).connect(config.username)
        ...
        .then((groups) => {
            ...
            filteredGroups = groups.filter((group) => group._data.type == 'Room' || group._data.type == 'Zone');
